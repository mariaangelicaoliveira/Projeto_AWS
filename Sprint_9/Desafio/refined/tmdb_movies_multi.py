...
def process_and_save_multidimensional(input_path, fact_output_path, dim_output_paths):
    try:
        ...
        dim_title_df = df.select(
            ...
            col("title").alias("principal_title"),
        ...
        dim_year_df = df.select(
            col("release_date").alias("release_year")
        ).dropDuplicates()
...
parquet_input_path = "s3://bucketdesafio/trusted_zone/tmdb/movies/dt=2024-07-28/"
fact_output_path = "s3://bucketdesafio/refined_zone/tmdb_movies/fato_movies/"
dim_output_paths = {
    'title': "s3://bucketdesafio/refined_zone/tmdb_movies/dim_title/",
    'year': "s3://bucketdesafio/refined_zone/tmdb_movies/dim_year/",
    'genre': "s3://bucketdesafio/refined_zone/tmdb_movies/dim_genre/",
    'language': "s3://bucketdesafio/refined_zone/tmdb_movies/dim_language/"
}
